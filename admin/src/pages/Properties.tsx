...
function HostsPanel({ propertyId, onCountChange }: { propertyId: string; onCountChange: (n: number) => void }) {
  ...
  const [error, setError] = useState('');
  // Inline PIN editor: hostId → current input value
  const [pinEditing, setPinEditing] = useState<Record<string, string>>({});
  ...
  const handleAdd = async () => {
    ...
    try {
      await api.hosts.add(propertyId, name.trim(), phone.trim(), pin.trim() || undefined);
  ...
  const handleSetPin = async (hostId: string) => {
    const newPin = pinEditing[hostId]?.trim();
    if (!newPin || newPin.length < 4) return;
    setPinSaving(hostId);
    try {
      await api.hosts.setPin(propertyId, hostId, newPin);
      setPinEditing((p) => { const n = { ...p }; delete n[hostId]; return n; });
      setPinSuccess(hostId);
      setTimeout(() => setPinSuccess(null), 2500);
      void load();
    } catch (e) { setError(String(e)); }
    finally { setPinSaving(null); }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        ...
        <div className="flex gap-3 items-end flex-wrap">
          ...
          <div className="w-28">
            <label className="block text-xs text-slate-400 font-medium mb-1.5">PIN (for app login)</label>
            <input
              type="password"
              value={pin}
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-slate-100">
              <tr className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
                ...
                <th className="px-5 py-3.5 text-left">PIN</th>
              ...
              {hosts.map((h, i) => (
                <tr key={h.id} className={`${i > 0 ? 'border-t border-slate-50' : ''}`}>
                  ...
                  <td className="px-5 py-3">
                    {pinEditing[h.id] !== undefined ? (
                      <div className="flex items-center gap-2">
                        <input
                          type="password"
                          autoFocus
                          value={pinEditing[h.id]}
                          onChange={(e) => setPinEditing((p) => ({ ...p, [h.id]: e.target.value }))}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') void handleSetPin(h.id);
                            if (e.key === 'Escape') setPinEditing((p) => { const n = { ...p }; delete n[h.id]; return n; });
                          }}
                          maxLength={8}
                          placeholder="4–8 digits"
                          className="w-28 border border-slate-200 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-transparent"
                        />
                        <button
                          onClick={() => void handleSetPin(h.id)}
                          disabled={pinSaving === h.id || (pinEditing[h.id]?.length ?? 0) < 4}
                        ...
                        <button
                          onClick={() => setPinEditing((p) => { const n = { ...p }; delete n[h.id]; return n; })}
                      ...
                      <button
                        onClick={() => setPinEditing((p) => ({ ...p, [h.id]: '' }))}
